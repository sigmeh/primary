...
def graph_journal_timelines(journal_timelines,num_files):
	...
	label_color='blue'
	plt.title(plot_names,color=label_color)
	plt.xlabel('time',fontsize=14,color=label_color)
	plt.ylabel('% name (all journals)',fontsize=14,color=label_color)
	ax = plt.subplot()
...
def main():
	...
	plt.title('avg. % candidate name appearances from news sources')
	plt.xlabel('chronology')
	plt.ylabel('avg. % occurrence')
	plt.tight_layout()
	plt.show()
